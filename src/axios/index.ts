...
//     await Promise.all([
//         DB.getOne<{value:string}[]>(StoreName.STORE, ACCESS_TOKEN).then((res) => access_token = res ? res[0]?.value : ''),
//         DB.getOne<{value:string}[]>(StoreName.STORE, REFRESH_TOKEN).then((res) => refresh_token = res ? res[0] ?.value : '')
//     ])
//         .catch(console.error)
// }
//
