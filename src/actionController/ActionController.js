...
export default class ActionController {
  ...
  constructor(dbSchema, options) {
    ...
    !options.newAction &&
      console.warn(
        '[ActionController] you need to specify a function "newAction"'
      );
    /**@returns {ActionType} */
  ...
  _subscriptionsCall(action, data) {
    ...
    if (this.model[entity] && this.model[entity][actionVariant]) {
      ...
      this.subscriptions[entity][actionVariant].forEach((sub) => sub(data));
    }
  }
  ...
  write(payload) {
    if (this.modelNames.includes(payload.storeName)) {
      ...
      this._errorMessage(
        new Error(
          `[ActionController] storeName "${payload.storeName}" not correct`
        )
      );
