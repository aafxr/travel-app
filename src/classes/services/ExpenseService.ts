...
export class ExpenseService {
    ...
    static async create(expense: Partial<ExpenseType> | undefined, variant: ExpenseVariantType = "expenses_actual") {
        let newExpense: Expense
        if(expense) {
            if(variant === "expenses_actual") newExpense = new ExpenseActual(expense)
            else newExpense = new ExpensePlan(expense)
        } else {
